...
try {	
	...
	ArrayList mySuspectUpRequirements = ProjectUtil.getSuspectUpRequriements(securityProfile, user.getEmailId(), databaseType, project.getProjectId(), dashboardType, ownedBy);
%>

<table class='paddedTable'  width='100%'>
	<tr>
		<td colspan='7' align='center'>
			<span class='normalText'><b>Items that have a suspect trace upstream</b></span>
		</td>
	</tr>
	<tr>
		<td colspan='7' align='right'>
			<input type='button' class='btn btn-danger btn-sm' onclick='displayMyTasksForAProject();' value='  Return to My Tasks  '>
			</input>
		</td>
	</tr>
<%
	...
	Iterator sUR = mySuspectUpRequirements.iterator();
	while (sUR.hasNext()){
		Requirement r = (Requirement) sUR.next();
		...
		<td >
			...
			<td>
				...
				%>
					<div id='clearSuspectUpStreamDiv<%=r.getRequirementId()%>'>
						<span class='normalText'>
							<input type='button' class='btn btn-primary btn-sm' value='Clear all Suspects Upstream' 
								onclick='clearSuspectsUpStreamForMyTasks(<%=r.getRequirementId()%>);'></input>
						</span>
					</div>
				<%} %>
			</td>
