...
try {	
	...
	int myOrphanRequirements = ProjectUtil.getOrphanRequriementsCount(user.getEmailId(), targetProjectId,  dashboardType, ownedBy);
	if (myOrphanRequirements == 0 ){
%>
	
	<span style="color:gray; font-size:15pt"><b><%=myOrphanRequirements %></b>  </span><span class='normalText' style='color:gray'> <br></br>Orphan</span>
<%
	}
	else {
%>
	<a href='#' onclick='myOrphanReqsDetails(<%=targetProjectId%>, "<%=dashboardType%>","<%=ownedBy%>")'>
	<span style="color:red; font-size:15pt"><b><%=myOrphanRequirements %></b></SPAN> <br></br>Orphan</a>
<%		
	}
}
