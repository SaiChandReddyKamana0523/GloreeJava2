...
try {
	...
	int myIncompleteRequirements = ProjectUtil.getIncompleteRequriementsCount(user.getEmailId(), targetProjectId ,  dashboardType, ownedBy);


	if (myIncompleteRequirements == 0 ){
%>
	<span style="color:gray; font-size:15pt"><b><%=myIncompleteRequirements %></b>  </span><span class='normalText' style='color:gray'>  <br></br>Incomplete</span>
<%
	}
	else {
%>		
	<a href='#' onclick='myIncompleteReqsDetails(<%=targetProjectId%> , "<%=dashboardType%>","<%=ownedBy%>")'>
	<span style="color:red; font-size:15pt"><b><%=myIncompleteRequirements %></b></span> <br></br>Incomplete</a>	
<%		
	}
}
