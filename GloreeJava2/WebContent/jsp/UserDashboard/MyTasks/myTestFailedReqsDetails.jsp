...
try {	
	...
	ArrayList myTestFailedRequirements = ProjectUtil.getTestFailedRequriements(securityProfile, user.getEmailId(), databaseType, targetProjectId,  dashboardType, ownedBy);
%>
	...
	<tr>
		<td colspan='7' align='center'>
			<%if (ownedBy.equals(user.getEmailId())) { %>
				<span class='normalText'><b>My Items that have failed testing</b></span>
			<%}
			else {
			%>
				<span class='normalText'><b><%=ownedBy %>'s Items that have failed testing</b></span>
			<%
			}%>
		</td>
	</tr>
...
<%
	...
	Iterator tFR = myTestFailedRequirements.iterator();
	
	while (tFR.hasNext()){
		Requirement r = (Requirement) tFR.next();
