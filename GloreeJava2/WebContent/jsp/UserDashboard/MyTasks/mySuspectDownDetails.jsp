...
try {	
	...
	ArrayList mySuspectDownRequirements = ProjectUtil.getSuspectDownRequriements(securityProfile, user.getEmailId(), databaseType, targetProjectId , dashboardType, ownedBy);
%>
	...
	<tr>
		<td colspan='7' align='center'>
			<%if (ownedBy.equals(user.getEmailId())) { %>
				<span class='normalText'><b>My items that have a suspect trace downstream</b></span>
			<%}
			else {
			%>
				<span class='normalText'><b><%=ownedBy %>'s items that have a suspect trace downstream</b></span>
			<%
			}%>		
		</td>
	</tr>
...
<%
	...
	Iterator sDR = mySuspectDownRequirements.iterator();
	while (sDR.hasNext()){
		Requirement r = (Requirement) sDR.next();
		...
		<td >
			...
			<td>
				<%
				if (!(securityProfile.getPrivileges().contains("traceToRequirementsInFolder" + r.getFolderId()))){
					// do nothing
				
		    	}
				else {
				%>
					<div id='clearSuspectDownStreamDiv<%=r.getRequirementId()%>'>
						<span class='normalText'>
							<input type='button'   class='btn btn-primary btn-sm' value='Clear all Suspect Downstream' 
							onclick='clearSuspectsDownStreamForMyTasks(<%=r.getRequirementId()%>);'></input>
						</span>
					</div>
				<%} %>
			</td>
