...
try {	
	...
	int myRequirementsPendingApproval = ProjectUtil.getReqsPendingApprovalCount(user.getEmailId(),  targetProjectId ,  dashboardType, ownedBy);
	if (myRequirementsPendingApproval == 0 ) {
%>
	<span style="color:gray; font-size:15pt"><b><%=myRequirementsPendingApproval %></b>  </span><span class='normalText' style='color:gray'>  <br></br>Pending Approval</span>
<%
	}
	else {
%>
	<a href='#' onclick='myReqsPendingApprovalDetails(<%=targetProjectId%>, "<%=dashboardType%>","<%=ownedBy%>")'>
	<span style="color:red; font-size:15pt"><b><%=myRequirementsPendingApproval %></b></SPAN> <br></br>Pending Approval</a>
<%		
	}
}
