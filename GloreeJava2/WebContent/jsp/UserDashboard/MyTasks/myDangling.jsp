...
try {	
	...
	int danglingRequirements = ProjectUtil.getDanglingRequriementsCount(user.getEmailId(), targetProjectId,  dashboardType, ownedBy);
	if (danglingRequirements == 0 ){
%>
	<span style="color:gray; font-size:15pt"><b><%=danglingRequirements %></b> </span><span class='normalText' style='color:gray'> <br></br>Dangling</span>
<%
	}
	else {
%>
	<a href='#' onclick='danglingReqsDetails(<%=targetProjectId%> ,"<%=dashboardType%>","<%=ownedBy%>")'>
	<span style="color:red; font-size:15pt"><b><%=danglingRequirements %></b></SPAN> <br></br>Dangling</a>
<%		
	}
}
