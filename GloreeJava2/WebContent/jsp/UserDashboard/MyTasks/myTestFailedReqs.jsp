...
try {	
	...
	int myTestFailedRequirements = ProjectUtil.getTestFailedRequriementsCount(user.getEmailId(), targetProjectId,  dashboardType, ownedBy);
	
	if (myTestFailedRequirements == 0 ) {
%>
	<span style="color:gray; font-size:15pt"><b><%=myTestFailedRequirements %></b>  </span><span class='normalText' style='color:gray'>  <br></br>Test Failed</span>
<%
	}
	else {
%>
	<a href='#' onclick='myTestFailedReqsDetails(<%=targetProjectId%>,"<%=dashboardType%>","<%=ownedBy%>")'>
	<span style="color:red; font-size:15pt"><b><%=myTestFailedRequirements %></b></SPAN> <br></br>Test Failed</a>
<%		
	}
}
