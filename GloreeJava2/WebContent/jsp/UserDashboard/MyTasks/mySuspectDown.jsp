...
try {	
	...
	System.out.println("srt about to call suspect down");
	int mySuspectDownStreamRequirements = ProjectUtil.getSuspectDownStreamRequriementsCount(user.getEmailId(), targetProjectId ,  dashboardType, ownedBy);
	if (mySuspectDownStreamRequirements == 0 ){
%>
	<span style="color:gray; font-size:15pt"><b><%=mySuspectDownStreamRequirements %></b>  </span><span class='normalText' style='color:gray'> <br></br>Suspect Downstream</span>
<%
	}
	else {
%>
	<a href='#' onclick='mySuspectDownDetails(<%=targetProjectId%> ,"<%=dashboardType%>","<%=ownedBy%>") '><span style="color:red; font-size:15pt"><b><%=mySuspectDownStreamRequirements %></b></span><br></br>Suspect Downstream</a>
<%		
	}
}
