...
try {	
	...
	int myRequirementsRejected = ProjectUtil.getReqsRejectedCount(user.getEmailId(), targetProjectId,  dashboardType, ownedBy);
	if (myRequirementsRejected == 0 ){
%>
	<span style="color:gray; font-size:15pt"><b><%=myRequirementsRejected %></b>  </span><span class='normalText' style='color:gray'>  <br></br>Rejected</span>
<%
	}
	else {
%>
	<a href='#' onclick='myReqsRejectedDetails(<%=targetProjectId%>, "<%=dashboardType%>","<%=ownedBy%>")'>
	<span style="color:red; font-size:15pt"><b><%=myRequirementsRejected %></b></span> <br></br>Rejected</a>
<%		
	}
}
