...
try {	
	...
	ArrayList mRequirementsRejected = ProjectUtil.getRequirementsThatAreRejected(securityProfile, user.getEmailId(), databaseType, targetProjectId , dashboardType, ownedBy);
%>
	...
	<tr>
		<td colspan='7' align='center'>
			<%if (ownedBy.equals(user.getEmailId())) { %>
				<span class='normalText'><b>My Requirements that have been rejected by other approvers</b></span>
			<%}
			else {
			%>
				<span class='normalText'><b><%=ownedBy %>'s Requirements that have been rejected by other approvers</b></span>
			<%
			}%>		
...
<%
	...
	Iterator dR = mRequirementsRejected.iterator();
