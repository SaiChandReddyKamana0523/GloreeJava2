...
<% }
	...
	<%		
	...
	else if (exportToExcel.equals("true")){
		System.out.println("about to export to Excel");
		// lets set the session attributes.
		
		session.setAttribute("tracePanelFromRequirements", fromRequirements);
		session.setAttribute("tracePanelToRequirements", toRequirements);
			
		RequestDispatcher dispatcher =	request.getRequestDispatcher("/servlet/ReportAction?action=exportTracePanel");
		...
		for (int i=1;i<=numOfToPages;i++){
			if (i == toPageToDisplay){
				...
				toPageString += "<a href='#' onclick='fillTracePanel(" + fromPageToDisplay + "," + i + ", \"readOnly\"); '> " + i +" </a> ";
				toPageString += "&nbsp;&nbsp;|&nbsp;&nbsp; ";	
			}
		}
		// drop the last nbsp;
		...
		for (int i=1;i<=numOfFromPages;i++){
			if (i == fromPageToDisplay){
				...
				fromPageString += "<a href='#' onclick='fillTracePanel(" + i+ "," + toPageToDisplay + ", \"readOnly\"); '> " + i +" </a> ";
				fromPageString += "&nbsp;&nbsp;|&nbsp;&nbsp; ";	
			}
		}
		// drop the last nbsp;
	...
	%>
		...
		<table class='table' border=6 bordercolor='lightgray' >
		<% 
		////////////////////////////////////////////////////////////////////////////////////////////////
		...
		<tr>
			<td style="width:300px" align='center'>
							...
							<div class='alert alert-danger'>
								...
								&nbsp;&nbsp;&nbsp;&nbsp;To Reqs &nbsp; <img src="/GloreeJava2/images/arrow_right.png"  width="16" border="0">
								</span>
							</div>
		...
		while (toI.hasNext()){
			...
			%>
			<td valign='top' style='height:100%; text-align:center' align='middle' class='warning'>
				<div >
		...
		while (fromI.hasNext()){
				...
				
			{
				...
				%>
				<tr>
					<td style="width:300px" class='warning' >
						<div >		
				...
				while (toI.hasNext()){
					...
					%>
					<td align='center'  title='<%=traceTitle %>' id='<%=fromRequirementId%>-<%=toRequirementId%>'  class='danger'>
					...
					%> 
					<td align='center' title='<%=traceTitle %>' id='<%=fromRequirementId%>-<%=toRequirementId%>' class='success' >
					...
					%>
					<td align='center'  title='<%=traceTitle %>'  id='<%=fromRequirementId%>-<%=toRequirementId%>' class='active'>
					</td>
