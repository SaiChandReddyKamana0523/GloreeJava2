...
<%if (dRIsMember){ 
	...
	// lets get the list of distinct req types in this folder.
	
	ArrayList reqTypes = ReleaseMetricsUtil.getRequirementTypesForFolder(folderId,project.getProjectId());
	
	String allRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfRequirements", databaseType);
	String draftRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfDraftRequirements", databaseType);
	String inApprovalWorkflowRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfInApprovalWorkflowRequirements", databaseType);
	String rejectedRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfRejectedRequirements", databaseType);
	String approvedRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfApprovedRequirements", databaseType);
	String danglingRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfDanglingRequirements", databaseType);
	String orphanRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfOrphanRequirements", databaseType);
	String suspectUpstreamRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfSuspectUpstreamRequirements", databaseType);
	String suspectDownstreamRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfSuspectDownstreamRequirements", databaseType);
	String completedRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfCompletedRequirements", databaseType);
	String incompleteRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfIncompleteRequirements", databaseType);
	
	String testPendingRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfTestPendingRequirements", databaseType);
	String testPassRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfTestPassRequirements", databaseType);
	String testFailRequirementsDataString = ReleaseMetricsUtil.getTrendDataStringForFolder(folderId,project.getProjectId(), "numOfTestFailRequirements", databaseType);
	

	String defectRequirementsDataString = ReleaseMetricsUtil.getDefectTrendDataString("folder",folderId, databaseType);
	ArrayList defectStatusGroupsForProject = ReleaseMetricsUtil.getDefectStatusGroupsInMetrics("folder", folderId);	
...
<body id="yahoo-com" class=" yui-skin-sam">
	...
	<div class='level1Box' style="text-align:left">		
		...
		<span class="chart_title">Folder Trends
		
			<a href='/GloreeJava2/servlet/ReportAction?action=exportFolderTrendData&folderId=<%=folderId%>'
				target='_blank'  title='Excel file of Folder Trends Data'>
		...
		//--- data
			...
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'All',
					content: '<span class="chart_title">All Requirements in Folder</span><div class="chart" id="allRequirementsChart"></div>',
					active: true
			}));
			...
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Draft',
					content: '<span class="chart_title">Draft Requirements in Folder</span><div class="chart" id="draftRequirementsChart"></div>'
			}));
		
			//Add a tab for the in approval work flow Chart
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Pending',
					content: '<span class="chart_title">In Approval Workflow Requirements in Folder</span><div class="chart" id="inApprovalWorkflowRequirementsChart"></div>'
			}));
		
			//Add a tab for the rejected reqs Chart
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Rejected',
					content: '<span class="chart_title">Draft Requirements in Folder</span><div class="chart" id="rejectedRequirementsChart"></div>'
			}));
		
			//Add a tab for the approved reqs Chart
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Approved',
					content: '<span class="chart_title">Draft Requirements in Folder</span><div class="chart" id="approvedRequirementsChart"></div>'
			}));
			...
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Dangling',
					content: '<span class="chart_title">Dangling (without a child) Requirements in Folder</span><div class="chart" id="danglingRequirementsChart"></div>'
			}));
		
			//Add a tab for the orphan reqs Chart
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Orphan',
					content: '<span class="chart_title">Orphan (without a parent) Requirements in Folder</span><div class="chart" id="orphanRequirementsChart"></div>'
			}));
			//Add a tab for the suspectUpstream reqs Chart
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Suspect Upstream',
					content: '<span class="chart_title">Requirements with a suspect trace upstream in Folder</span><div class="chart" id="suspectUpstreamRequirementsChart"></div>'
			}));
		
			//Add a tab for the suspectDownstream reqs Chart
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Suspect Downstream',
					content: '<span class="chart_title">Requirements with a suspect trace downstream in Folder</span><div class="chart" id="suspectDownstreamRequirementsChart"></div>'
			}));
		
		
			<% if (folder.getRequirementTypeName().equals("Defects")){  
				// For folders that don't belont to req type Defects there won't be any defects requirements
				// so no point showing the trend graph.
			%>
			tabView.addTab( new YAHOO.widget.Tab({
				label: 'Defects',
				content: '<span class="chart_title">Defects in Folder</span><div class="chart" id="defectRequirementsChart"></div>'
			}));
			<%}%>
			...
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Completed',
					content: '<span class="chart_title">Completed Requirements in Folder</span><div class="chart" id="completedRequirementsChart"></div>'
			}));
			...
			tabView.addTab( new YAHOO.widget.Tab({
					label: 'Incomplete',
					content: '<span class="chart_title">Incomplete Requirements in Folder</span><div class="chart" id="incompleteRequirementsChart"></div>'
			}));
