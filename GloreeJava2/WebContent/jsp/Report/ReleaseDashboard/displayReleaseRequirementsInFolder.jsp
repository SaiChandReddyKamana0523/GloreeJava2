...
%>
	...
	<%
		...
		ArrayList releaseRequirements  = ReleaseMetricsUtil.getRequirementsForReleaseOrProject(securityProfile,
			releaseId,
		...
		ArrayList defaultDisplayAttributes = displayFolder.getDefaultDisplayAttributes();
		int folderEnabledForApproval = displayFolder.getIsFolderEnabledForApproval();
					...
					%>
							<tr>
					    		...
					    		<td style='width:600px; text-align:left' onmouseover="this.style.backgroundColor='pink';" onmouseout="this.style.backgroundColor='white'">
					    			<span class='sectionHeadingText'><%=displayFolder.getRequirementTypeName()%> Name</span>
					    		</td>
					    		<%
					    		...
					    		%>
					    		<td><span class='sectionHeadingText'>Action</span></td>
					    		<%
									 ...
									 if (securityProfile.getPrivileges().contains("readRequirementsInFolder" + r.getFolderId())){
										...
										<%
											...
											<td style='width:100px' align='right'>
													<span class='normalText'>
														<select id='requirementAction<%=r.getRequirementId()%>'  style='visibility:hidden'
